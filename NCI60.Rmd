...
En el caso de querer hacer un análisis supervisado, además también necesitará un factor referente a la clase a la que pertenece cada muestra, así como dividir el conjunto de datos en "training set" y "test set". En este caso voy a aplicar un análisis supervisado, siendo la variable "tipo de cáncer" la que define los grupos. La idea sería también separar las muestras en 2/3 para el "training set" y 1/3 para el "test set", pero debido al elevado número de grups (n=9) y al relativo bajo número de muestras por grupo (2, 5 y hasta un máximo de 10), se considera oportuno no hacer esta división, por lo que no será posible validar el modelo, y los resultados obtenidos servirán para generar hipótesis que deberán ser posteriormente validadas con nuevas muestras.
...
test.keepX = list(
  agilent = seq(2,20,10), 
  hgu133 = seq(2,20,10), 
  hgu133p2 = seq(2,20,10), 
  hgu95 = seq(2,20,10))
set.seed(123)
tune = tune.block.splsda(
  X = NCI60_t, Y = cancer_type, ncomp = 4, test.keepX = test.keepX, 
  design = matrix(1, ncol = length(NCI60_t), nrow = length(NCI60_t), 
                  dimnames = list(names(NCI60_t), names(NCI60_t))), 
  #nrepeat = 3
   validation = "loo")
tune$choice.keepX
tmp <- data.frame(tune$error.rate)
tmp[order(tmp$comp1, tmp$comp2, tmp$comp3, tmp$comp4),]
```
...
```{r}
list.keepX = list(
  agilent =  c(02,12,12,12), 
  hgu133 =   c(12,02,02,02), 
  hgu133p2 = c(02,02,12,02), 
  hgu95 =    c(12,02,12,02)
  )

res.diablo <- block.splsda(
  NCI60_t, cancer_type, keepX = list.keepX, 
...
perf.diablo <- perf(res.diablo, validation = 'loo', 
                      nrepeat = 10, 
                      dist = 'centroids.dist')
perf.diablo$MajorityVote.error.rate
```

Parece ser que la primera componente separa las muestras LE del resto de muestras (almenos es el grupo con una menor tasa de error en esta componente), la segunda componente separaría sobretodo las muestras ME, pero también las CO y de nuevo LE. La tercera componente separaría principalmente de nuevo las muestras ME (ya separadas en la segunda componente), pero también las LE (separadas por la primera y la segunda componentes) y CNS, y la cuarta componente separaría perfectametne las muestras RE, pero también las ME, CNS, CO y LE. Parecería que las muestras LC, OV, BR y PR serían aquellas que no se consiguen separar con éxito por ninguna de las primeras cuatro dimensiones (haría falta ver qué pasa en las siguientes componentes).

### AUC

Las AUCs se dibujan por cada bloque de variables por separado, pero yo estoy  más interesada en observar los resultados considerando todos los bloques a la vez, por lo que por ahora no voy a realizar estos gráficos.
...
Solucionar el problema de los simbolos: probar de instalar la version: `BiocManager::install('ajabadi/mixOmics@devel', update = TRUE, ask = FALSE)` (https://mixomics-users.discourse.group/t/pch-error-in-plotindiv/403/2).  
  
Se observa/confirma como el primer componente principalmente separa las muestras LE y el segundo componente las muestras ME.  

```{r}
plotIndiv(res.diablo, blocks = "consensus", ellipse = TRUE, legend = TRUE, comp = c(3,4))
```

Parece que el tercer componente estaría separando las muestras CO, mientras que el cuarto las muestras RE.
...
plotVar(res.diablo, legend = TRUE, comp = c(3, 4))
plotLoadings(res.diablo, comp = 1, contrib = "max")
plotLoadings(res.diablo, comp = 2, contrib = "max")
plotLoadings(res.diablo, comp = 3, contrib = "max")
plotLoadings(res.diablo, comp = 4, contrib = "max")
plotDiablo(res.diablo, ncomp = 1)
plotDiablo(res.diablo, ncomp = 2)
plotDiablo(res.diablo, ncomp = 3)
plotDiablo(res.diablo, ncomp = 4)
#circosPlot(res.diablo, cutoff = 0.7)
jpeg(filename="nci60_cim_c1.jpeg")
cimDiablo(res.diablo, 
          color.blocks = c('darkorchid', 'brown1', 'lightgreen', "blue"), 
          comp = 1, margin = c(8,20), legend.position = "right")
dev.off()
jpeg(filename="nci60_cim_c2.jpeg")
cimDiablo(res.diablo, 
          color.blocks = c('darkorchid', 'brown1', 'lightgreen', "blue"), 
          comp = 2, margin = c(8,20), legend.position = "right")
dev.off()
jpeg(filename="nci60_cim_c3.jpeg")
cimDiablo(res.diablo, 
          color.blocks = c('darkorchid', 'brown1', 'lightgreen', "blue"), 
          comp = 3, margin = c(8,20), legend.position = "right")
dev.off()
jpeg(filename="nci60_cim_c4.jpeg")
cimDiablo(res.diablo, 
          color.blocks = c('darkorchid', 'brown1', 'lightgreen', "blue"), 
          comp = 4, margin = c(8,20), legend.position = "right")
dev.off()
#network(res.diablo, blocks = c(1,2,3,4),
#        color.node = c('darkorchid', #'brown1', 'lightgreen', "blue"), 
#        cutoff = 0.7, save = 'jpeg', name.save = 'nci60_network')
```
![](/Users/lenovo/Documents/GitHub/UOC_TFM_omicsintegration/nci60_cim_c1.jpeg)
![](/Users/lenovo/Documents/GitHub/UOC_TFM_omicsintegration/nci60_cim_c2.jpeg)
![](/Users/lenovo/Documents/GitHub/UOC_TFM_omicsintegration/nci60_cim_c3.jpeg)
![](/Users/lenovo/Documents/GitHub/UOC_TFM_omicsintegration/nci60_cim_c4.jpeg)

![](/Users/lenovo/Documents/GitHub/UOC_TFM_omicsintegration/nci60_network.jpeg)
