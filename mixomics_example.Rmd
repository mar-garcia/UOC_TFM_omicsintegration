...
`list(mRNA = c(30, 10), miRNA = c(20, 30), protein = c(1, 6))`   
Es decir, el panel multiómico óptimo consistiría en 30 mRNAs, 20 miRNAs y 1 proteína para el componente 1, y 10 mRNAs, 30 miRNAs, 5 CpGs y 6 proteínas para el componente 2. Es decir, un total de 40 mRNAs, 50 miRNAs y 7 proteínas.  
   
De todas formas me surgen las siguientes dudas:  
  
  * Según `tune$choice.keepX`, para el primer componente el "keep" óptimo sería 30-10-1 (mrna-mirna-protein), pero viendo `tune$error.rate` interpreto que el "keep" óptimo puede ser qualquiera de las siguientes 3 combinaciones ya que el error rate siempre es igual (0.333): 30-10-1, 30-20-1 y 30-10-6. Interpreto que el algoritmo ha escogido 30-10-1 porqué es la primera combinación con el mínimo "error rate" para el "comp1". ¿Es correcta esta interpretación?  
  * En un primer momento he escogido la combinación 30-20-1 para el primer componente, porqué de las 3 combinaciones con el mismo "error rate" para el C1 es que el que presenta un menor "error rate" para el C2. De todas formas, me surge la duda de si fuera mejor escoger la combinación 30-10-6 por el hecho de que así no se restringue a 1 sola variable el conjunto de proteínas. ¿Estás de acuerdo que, a igual "error rate" para C1, aunque el menor "error rate" para el C2 fuera 30-20-1 (siempre hablando de C1), sería mejor indicar 30-10-6 por el hecho de no restringuirse ya desde el principio a la selección de 1 sola variable para 1 conjunto de datos?  
