...
Los datos deben estar en una lista compuesta por distintos "data frames", una por cada tipo de datos. En el caso de querer hacer un análisis supervisado, además también necesitará un factor referente a la clase a la que pertenece cada muestra. Las muestras se situan en las filas y las variables en las columnas.  
`DIABLO` necesita una variable respuesta. En este caso voy a usar la variable "Overall.quality".
...
sapply(X, dim)
Y <- matrix(wine$Overall.quality)
rownames(Y) <- rownames(wine)
res.diablo <- block.pls(X = X, Y = Y, ncomp = 2, 
                        scale = TRUE)
```
...
plot(res.diablo$weights$comp1, res.diablo$weights$comp2, 
     xlim = c(0,max(res.diablo$weights$comp1)+0.1), 
     ylim = c(0,max(res.diablo$weights$comp2)+0.1), col = 1:4, pch=16,
     xlab = "Dim 1", ylab = "Dim 2")
grid()
abline(v=0, lty = 2)
abline(h=0, lty = 2)
text(res.diablo$weights$comp1, res.diablo$weights$comp2, rownames(res.diablo$weights), pos = 1, col = 1:4, cex = 0.8)
```

También aquí se observa como los cuatro conjutnos de datos presentan un peso similar en la primera componente. Sin embargo, la distribución de los pesos en la segunda dimensión aquí ya difiere un poco respeto a los algoritmos precedentes.  

### Variables individuales

```{r}
tmp <- do.call("rbind", res.diablo$loadings)
tmp <- data.frame(tmp[-nrow(tmp),])

plot(tmp$comp1, tmp$comp2, col = c(rep(1, 5), rep(2, 10), rep(3, 3), rep(4, 9)), pch = 16,
     xlab = "Dim 1", ylab = "Dim 2", xlim = c(min(tmp$comp1)-0.1,
                                              max(tmp$comp1)+0.1),
     ylim = c(min(tmp$comp2)-0.1,
                                              max(tmp$comp2)+0.1))
grid()
abline(v=0, lty = 2)
abline(h=0, lty = 2)
text(tmp$comp1, tmp$comp2, rownames(tmp),pos = 1, cex = 0.5)
legend("topright", col = 1:4, pch = 16, legend = names(X))
```


No se permite hacer el círculo de correlaciones (`plotVar`) para un modelo no supervisado. Tampoco se pueden hacer `circosPlot` ni `cimPlot`.
...
network(res.diablo, blocks = c(1,2,3,4), cutoff = 0.9, save = "jpeg", name.save = "wines_network")
```

![](/Users/lenovo/Documents/GitHub/UOC_TFM_omicsintegration/wines_network.jpeg)

## Muestras

```{r}
plotIndiv(res.diablo, blocks = "consensus")
```

Como en el caso del `MFA` se observa como la primera dimensión separa el vino `1DAM` de los vinos `2ING` y `1VAU`, y que la segunda dimensión separa los vinos `T1` y `T2`.  
También en este caso (en concordancia con el `MFA`), las variables con más peso en la primera dimensión son las 3 variables que miden características visuales, así como la calidad del aroma y la frutabilidad antes de la agitación, mientras que en la segunda dimensión, aquellas variables con mayor peso también fueron "spice before shaking", "odor intensity before shaking", "biterness", "spice" y "odor intensity".  


# Conclusiones

- `DIABLO` permite la realización de análisis supervisados y no supervisados, mientras que los otros 2 algoritmos únicamente permiten análisis no supervisados. Referente a los análisis supervisados, `DIABLO` permite tener un "training set" y un "test set".    
...
- `MCIA` permite evaluar la correlación entre los distintos conjuntos de variables a través del coeficiente "RV".  
